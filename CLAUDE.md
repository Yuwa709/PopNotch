...
6. **No analytics, telemetry, or crash-phone-home, ever.** Network calls are allowed only in features that are inherently network-based, and only to the endpoint that feature needs: **weather** (Open-Meteo), **lyrics** (LRCLIB), **stocks** (Phase 6, provider TBD), and **update checks** (Sparkle, Phase 5). Adding a network call anywhere else requires an explicit decision recorded in `PROJECT-CONTEXT.md`.
